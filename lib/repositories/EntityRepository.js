class EntityRepository {
    ...
    async getGoalsByAssignment(userId) {
        try {
            let assignedGoals = await this._db('GoalAssignment')
                .join('Goal', 'GoalAssignment.goalId', '=', 'Goal.id')
                .where({userId});
    ...
    async getUsersByAssignment(goalId) {
        try {
            let assignedUsers = await this._db('GoalAssignment')
                .join('User', 'GoalAssignment.userId', '=', 'User.id')
                .where({goalId});
